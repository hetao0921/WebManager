...
public class LocalCenterDao {
	...
	public String query() {
		...
		List<HashMap<String, String>> list = ConnDo.getConnDo().executeQuery(
				hql);
	...
	public boolean isExsit(String centerID) {
		String hql = "select new Map(centerIp as centerIp) from MqLocalcenterinfotab where centerId = '"
				+ centerID + "'";
		List<HashMap<String, String>> list = ConnDo.getConnDo().executeQuery(
				hql);
		if (list != null && list.size() >= 1)
	...
	public LocalCenter queryInfo() {
		...
		centerInfo
				.setPort(Integer.parseInt(data.get("CenterPort".toLowerCase())));
		centerInfo.setSyncServerIP((null == data.get("SyncServerIP"
				.toLowerCase()) || ("").equals(data.get("SyncServerIP"
				.toLowerCase()))) ? data.get("centerip") : data
				.get("SyncServerIP".toLowerCase()));
		centerInfo.setSyncServerPort(Integer.parseInt(null == data
				.get("SyncServerPort".toLowerCase()) ? "5050" : data
				.get("SyncServerPort".toLowerCase())));
		centerInfo.setSyncServerPostAddress(ArgsUtil.getPostAddress());
		return centerInfo;
	}

	public int updateIpInfo(String centerIp, String centerGate,
			String centerMask) {
