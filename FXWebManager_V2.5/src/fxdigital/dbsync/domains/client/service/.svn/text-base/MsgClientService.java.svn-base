...
import fxdigital.dbsync.domains.client.dao.DataIncrementVersionDao;
...
public class MsgClientService implements IMessageListener {
	...
	public List<HashMap<String, String>> getLoadVersionInfo(
		...
		if (loadLock.get("flag").equals("1")) {
			...
			try {
				incrementList = DataIncrementVersionDao.getInstance()
						.getAllServerSource();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			...
			if (strlist.size() > 0) {
				...
				if (Integer.valueOf(localSelfVersion) < Integer
					...
					try {
						...
						DataSelfSourceDao.getInstance().recordAllStates(
	...
	public String sendLoadCommand(List<HashMap<String, String>> list,
		...
		String errorinfo = null;
		HashMap<String, String> loadLock = getLoadMsg();
		String loadlockid = loadLock.get("id");
		String centerid = null;
		String centername = null;
		String centerip = null;
		if (centerInfo.size() > 0) {
	...
	// 下载老文件的数据
	public boolean loadOldData(List<String> oldsqls) {
		boolean result = false;

		try {
			result = JdbcImpl.getJdbcImpl().updateSqls(oldsqls);
		} catch (Exception e) {
			logger.info("插入oldsqls失败。。。+e");
		}
	...
	public void processLoadData(DBSyncContent recontent) {
		...
		logger.info( "size  " + incrementList.size()+"incrementList---------" + incrementList );
		if (loadList.size() > 0 && null != loadList) {
			for (int i = 0; i < loadList.size(); i++) {
				logger.info( "size  " + loadList.size()+"loadList---------" + loadList );
				String centerid = loadList.get(i).get("centerid");
				...
				if (("").equals(oldXml) || null == oldXml) {
					logger.info("oldXml文件为空啊。。。");
				} else {
					logger.info("处理oldXml开始啦！！！");
					execResult = JdbcToXml.getInstance().xmlToInsert(oldXml,
							centerid);
					logger.info("旧全量数据全部执行完毕,返回结果：" + execResult);
					if (execResult) {
						logger.info("插入oldXml成功。。");
						if ((("").equals(newXml) || null == newXml)) {
							...
							dataNativeSource.setVersion(Integer
									.valueOf(version));
							Date d = new Date();
							...
							DataNativeSourceDao.getInstance().insert(
									dataNativeSource);
							oneoperateinfo = "下载成功";
						...
						logger.info("插入oldXml失败。。");
						errorinfo = "插入oldXml失败。。";
						loadState = "10";
						DataNativeRecordSourceDao.getInstance()
								.recordAllStates(centerid, centername,
										centerip, serverversion, uuid,
						...
						for(int i=0;i<incrementList.size();i++){
							...
							if (("").equals(localIncrementXml)
								...
								if (execResult) {
									...
									// 修改数据版本data_version
									DataIncrementVersionDao.getInstance().delete(
											localcenterid);
									...
									DataIncrementVersionDao.getInstance().insert(
											dataIncrementVersion);
								} else {
	 ...
	 * @return
	 */
	public List<HashMap<String, String>> getSearchLogsInfo(String centerid,String beginTime,String endTime) {
		List<HashMap<String, String>> resultList = new ArrayList<HashMap<String, String>>();
		String uuid = null;
		String sessionid = null;
		resultList = ViewLogs.getInstance().getSearchLogs(centerid,beginTime,endTime,uuid, sessionid
				);
		return resultList;
	...
	public boolean handle(IContent content) {
		...
		if (flag.equals(MsgType.AUTO_LOAD_SC)) {
			logger.info("接收下载增量信息");
			List<HashMap<String, String>> list = recontent.getList();
			processLoadData(recontent);
	...
	public BasicTelecomImpl getBasicTelecomImpl() {
		...
		if (null == bt) {
			logger.info("----------------listenr args----------------"
					+ initcenterid + " " + initmqip);
...
//			bt.setListener(msgService);
	...
	// 发送自增同步命令
	public void sendIncrementCommand(
			List<Map<String, Object>> incrementdataInfo, LocalCenter localCenter) {
				...
				while (true) {
					if (count < 5) {
						if (bt.send(NormalMode.MODE, content)) {
							...
							// 删除已经发送成功的项目
							IncrementdataInfotabDao.getInstance()
									.deleteIncrementdataID(id);
							break;
