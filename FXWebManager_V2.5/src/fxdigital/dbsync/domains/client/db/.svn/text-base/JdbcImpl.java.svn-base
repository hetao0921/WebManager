...
public class JdbcImpl {
	...
	public String[] getUpMsg() {
		...
		String sql = "select * from data_operate where operate = '上传'";
	...
	public String[] getUuid(String uuid, String operate) {
		DBConn db = DBConn.getDBConn();
		String sql = "select * from data_operate where operate = '" + operate
				+ "' and lockid='" + uuid + "'";
		int num = db.executeQuery(sql).size();
		if (num == 0) {
			return null;
	...
	public String[] getUpRecord(String uuid) {
		...
		String[] ss = db
				.executeQuery(
						"select * from data_operate_record where uuid = '"
								+ uuid + "'").get(0);
		// String userid = ss[2];
		return ss;
	}
	...
	public String[] getLoadMsg() {
		DBConn db = DBConn.getDBConn();
		String sql = "select * from data_operate where operate = '下载'";
		int num = db.executeQuery(sql).size();
		if (num == 0) {
			return null;
		} else {
	...
	public List<String[]> getAllSelfSource() {
		DBConn db = DBConn.getDBConn();
		return db.executeQuery("select * from data_self_source");
	}
...
//	}
	...
	public boolean updateUpState(String uuid, Integer flag) {
		...
		int num = db.executeQuery(sql).size();
		if (num == 0) {
			sql = "insert into data_operate(operate,flag,lockid) values('上传',1,'"
					+ uuid + "')";
		} else {
			if (flag == 1)
				sql = "update data_operate set flag =" + flag + ",lockid='"
		...
		sql = "select * from data_operate where operate='上传' and lockid='"
				+ uuid + "'";
	...
	public boolean updateLoadState(String uuid, Integer flag) {
		...
		int num = db.executeQuery(sql).size();
		if (num == 0) {
			sql = "insert into data_operate(operate,flag,lockid) values('下载',1,'"
					+ uuid + "')";
		} else {
			if (flag == 1)
				sql = "update data_operate set flag =" + flag + ",lockid='"
		...
		sql = "select * from data_operate where operate='下载' and lockid='"
				+ uuid + "'";
		if (db.executeQuery(sql).size() > 0)
			return true;
		else
	...
	public void insertUpRecord(String uuid, String userid) {
		DBConn db = DBConn.getDBConn();
		String sql = "insert into data_operate_record(uuid,operatorsessionid,operatetype) values('"
				+ uuid + "','" + userid + "','上传')";
		try {
			db.updateInfo(sql);
		} catch (Exception e) {
	...
	public void updateNativeRecord(Integer version, String centerid,
		...
		// "',"+version+",'"+uuid+"','"+state+"')";
		String sql = "update data_native_source set version=" + version
	...
	public void updateNativeRecord(String centerid, String uuid, Integer state) {
		...
		// "insert into data_native_source(centerid,version,uuid,flag) values('"+centerid+
		// "',"+version+",'"+uuid+"','"+state+"')";
		String sql = "update data_native_source set flag='" + state
	...
	public Integer getNativeFlagNum(String uuid) {
		// Integer num = -1;
		DBConn db = DBConn.getDBConn();
		String sql = "select * from data_native_source where uuid='" + uuid
				+ "' and flag='1'";
	...
	public void unLock(String uuid) {
		DBConn db = DBConn.getDBConn();
		String sql = "update data_operate set flag=0 where lockid='" + uuid
				+ "'";
		try {
			db.updateInfo(sql);
		} catch (Exception e) {
	...
	public void updateSelfSource(Integer version, String uuid, Integer flag) {
		...
		String sql = "select * from data_self_source";
		DBConn db = DBConn.getDBConn();
		if (db.executeQuery(sql).size() == 0) {
			sql = "insert into data_self_source(uuid,version,flag) values('"
					+ uuid + "'," + version + "," + flag + ")";
		} else {
			sql = "update data_self_source set flag=" + flag + ",uuid='" + uuid
					+ "',version=" + version;
		}
		System.out.println("SQL:" + sql);
		try {
	...
	public List<Map<String, String>> getAllNativeSource() {
		DBConn db = DBConn.getDBConn();
		String sql = "select * from data_native_source";
	...
	public void updateNativeSource(String centerid, Integer version, String uuid) {
		...
		if (getNativeSource(centerid) == 0) {
			sql = "insert into data_native_source(centerid,version,uuid,flag)"
					+ " values('" + centerid + "'," + 0 + ",'" + uuid
					+ "','1')";
		} else {
			sql = "update data_native_source set flag=1,uuid='" + uuid + "'"
	...
	public Integer getNativeSource(String centerid) {
		DBConn db = DBConn.getDBConn();
		String sql = "select * from data_native_source where centerid='"
				+ centerid + "'";
	...
	public void startInit() {
		...
		String sql1 = "delete from data_operate";
		String sql2 = "insert into data_operate(operate,flag) values('上传',0)";
		String sql3 = "insert into data_operate(operate,flag) values('下载',0)";
