	...
	public boolean xmlToInsert(String xml, String centerid) {
		...
		if(attribute!=null){
			...
			if("increment".equals(version)){
				...
				json =new String(Base64.decodeFast(json),Charset.forName("utf-8"));
				logger.info("increment 转码后:"+json);
				Element noticeEl=root.element("notice");
				String noticeJson=	noticeEl.getText();
				noticeJson =new String(Base64.decodeFast(noticeJson),Charset.forName("utf-8"));
...
public static void main(String[] args) {
	String str="<?xml version=\"1.0\" encoding=\"UTF-8\"?><TableMsg version=\"increment\" modelType=\"fxdigital.dbsync.domains.client.db.AnalysisGroup\" centerId=\"B8975A5FEA2B@001\"><increment>{\"groupId\":\"e4ea248a-5fb7-4cbc-a1e3-4abf0b6860f4\"}</increment><notice>{\"noticeContent\":\"e4ea248a-5fb7-4cbc-a1e3-4abf0b6860f4\",\"senderId\":\"\",\"senderType\":\"resourcegroupDelete\"}</notice></TableMsg>";
	JdbcToXml.getInstance().xmlToInsert(str,"");
	...
	public static String writeTheXml(String centerid)
	throws Exception  {
...
/*		String xml = "";
		...
		xml += "{TableMsg}"; 
		list = db.executeQueryXml("select * from nvmp.nvmp_datasynctab where SyncFlag = 1");
		String [] tableNames = new String[list.size()-2];
		for(int i =2;i<list.size();i++){
			tableNames[i-2] = list.get(i)[1];
		}
		for(int i =0;i<tableNames.length;i++){
			list = db.executeQueryXml("select * from nvmp."+tableNames[i]+" where centerid='"+centerid+"'");
			xml += "{"+tableNames[i]+"}";
...
//				list = db.executeQueryXml("select * from nvmp.nvmp_datasyncnewtab where SyncFlag = 1");
//				String[] tableNames = new String[list.size() - 2];
//				for (int i = 2; i < list.size(); i++) {
//					tableNames[i - 2] = list.get(i)[1];
//				}
...
//			}
	...
	public static String assemblyNewXml(String centerid)
			throws Exception  {
				...
				String xml = "";
				 DBConn db = DBConn.getDBConn();
				List<String[]> list = null;
				xml += "{TableMsg version='new' modelType='fxdigital.dbsync.domains.client.db.AnalysisNewXml' centerId='"+centerid+"'}"; 
				try {
					list = db.executeQueryXml("select * from nvmp.nvmp_datasynctab where SyncFlag = 1");
				} catch (Exception e) {
					logger.info("查询同步表nvmp_datasynctab出错，出错原因: "+e);
					throw new Exception("query nvmp_datasynctab error,error: "+e);
				}
				String [] tableNames = new String[list.size()-2];
				for(int i =2;i<list.size();i++){
					tableNames[i-2] = list.get(i)[1];
				...
				for(int i =0;i<tableNames.length;i++){
					try {
						...
						logger.info("查询同步表"+tableNames[i]+"出错，出错原因: "+e);
						throw new Exception("query "+tableNames[i]+" error,error: "+e);
					}
					xml += "{"+tableNames[i]+"}";
					String [] columnNames = list.get(0);
					xml += "{ColumnType}";
	 ...
	 * @throws SQLException
	 */
	public static String assemblyIncrementXml(String centerid,String json,Notice notice)
			throws SQLException  {
				...
				employees.addAttribute("modelType","fxdigital.dbsync.domains.client.db.AnalysisIncrementXml");
				employees.addAttribute("centerId",centerid);
	...
	public static String getLocalXml(Document document,String centerid){
		String path = "";
		String xml = "";
		List<String> pathList= readPathXML();
		if (System.getProperty("os.name").equals("Linux")) // 操作系统名称
		{
			path = "/"+pathList.get(1)+"/sync" + centerid + ".xml";
		} else {
			// logger.info("==============="+System.getProperty("user.dir"));
			path = pathList.get(0)+":\\sync" + centerid + ".xml";
		}
		...
		try {
		 ...
		 xml = LoadMessage.getInstance().readFileByLines(path);
		 logger.info("  "+"xml"+"  "+xml);
		 return xml;
	}
