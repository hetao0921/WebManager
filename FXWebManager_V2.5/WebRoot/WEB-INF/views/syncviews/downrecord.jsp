...
<script type="text/javascript">
			...
			jQuery("#jqGrid").jqGrid(
				{
					...
					height:"100%",
					autowidth  :true,
					colNames : [ '中心ID', '中心名称', '中心IP','本地版本','服务器版本','下载日期','下载状态'],
					colModel : [ {
						name : 'centerid',
						index : 'centerid',
						width : '20%',
    ...
    var json=getTdValue();
        ...
        if(data=="0"){
        messageDialog("下载锁定，请复位后再操作！", "info");
        getdownlock();
        }else if(data=="2"){
        	 downloading();
        }else{
         messageDialog("是最新数据！无需下载！", "info");
        getdownlock();
        }
        },"text");
    }
...
/*		if (confirm("确认复位？可能其他管理员正在进行下载操作！")) {
			$.post("undownlock", {
				"unlock" : "0"
				 ...
				 messageDialog(data, "info");
				getdownlock();
			});
		}*/
		
				confirmDialog("确认复位？可能其他管理员正在进行下载操作！",function(result){
				if(result){
					$.post("undownlock", {
				"unlock" : "0"
			}, function(data){
