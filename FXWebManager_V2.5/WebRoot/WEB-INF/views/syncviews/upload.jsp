		...
		document.getElementById('aftid').value="正在获取值。。。";
			if(data=="-1"){
				...
				uploading();
					...
					confirmDialog("确认上传？本地数据库收到最新版本，点击确认刷新数据!",function(result){
				if(result){
                  waitupload();
				}
			});
...
//			alert("未知错误！");
	...
	function partialrefresh(){
 	...
 	function reset() {
		confirmDialog("确认复位？可能其他管理员正在进行上传操作！",function(result){
				if(result){
					$.post("unlock", {
				"unlock" : "0"
			}, function(data){
		...
		<table class="tab" id="title_table">
					...
					<tr>
						<td>
							...
							<input type="text" id="uploadid" name="uploadversion" value="${selfversion}" readonly="readonly">
						</td>
