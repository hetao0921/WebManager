...
<script type="text/javascript">
	$(function() {
		jQuery("#jqGrid").jqGrid(
				{
					...
					mtype : "POST",
					autowidth : true,
					colNames : [ '用户名', '密码', '类型', '设备ID', '设备类型','设备类型', '输出类型',
							'IP地址', '端口号' ],
					colModel : [ {
						name : 'admin',
						index : 'admin',
		...
		function getDeviceTypes(selectedValue,devicetype){
			$.post("getDeviceTypes", function(data) {
				if (data != null && data != "") {
					...
					for ( var key in deviceTypes) {
						...
						if((value == selectedValue)&&(key==devicetype)){
							$("#deviceType option[text='"+key+"']").attr("selected","selected");
							flag = true;
						}
					}
					
					var count=$("#deviceType option").length;
					for(var i=0;i<count;i++)
					{
					    if($("#deviceType").get(0).options[i].text == devicetype)
					    {
					        $("#deviceType").get(0).options[i].selected = true;
					        break;
					    }
					}
		...
		$('#refresh_matrix').click(function(){
			var rowData =$("#jqGrid").getGridParam("reccount");
			if(rowData==0){
				messageDialog("没有矩阵可以刷新","notice");
				return;
			}
		...
		$('#edit').click(function(){
			...
			getDeviceTypes(rowData.deviceType,rowData.deviceTypev);
...
<body>
	...
	<div id="modal-dialog" style="display: none;">
		<form>
			<br>
			<label style="float: left;width: 70px">用&nbsp;户&nbsp;名：</label>
			<input id="user" type="text" name="txtUser"
				class="text ui-widget-content ui-corner-all"/>
			<br> <br>
			<label style="float: left;width: 70px">密&nbsp;&nbsp;&nbsp;&nbsp;码：</label>
			<input id="pass" type="text" name="txtPass"
				class="text ui-widget-content ui-corner-all" />
			<br> <br>
			<label style="float: left;width: 70px">类&nbsp;&nbsp;&nbsp;&nbsp;型：</label>
			<select id="class1" name="txtClass"><option>--[无数据]--</option></select>
			<br> <br>
			<label style="float: left;width: 70px">设&nbsp;备&nbsp;ID：</label>
			...
			<br> <br>
			<label style="float: left;width: 70px">IP&nbsp;地&nbsp;址：</label>
			<input id="address" type="text" name="txtAddress"
				class="text ui-widget-content ui-corner-all" />
			<br> <br>
			<label style="float: left;width: 70px">端&nbsp;口&nbsp;号：</label>
