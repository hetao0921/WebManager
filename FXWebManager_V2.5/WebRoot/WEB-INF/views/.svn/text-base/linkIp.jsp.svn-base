...
<script type="text/javascript">
	$(function() {
		...
		function doResize() {
			...
			var box = $("#localip_box").height();
			var buttonsHeight = $("#buttons").height();
			var tableHeadHeight = $("#jqgh_linkUpIp_id").height();
			var pagerHeight = $("#pglist").height();
			$("#linkUpIp").setGridHeight(mainContentHeight - box-buttonsHeight-pagerHeight-tableHeadHeight-8);
		...
		$("#edit_localip").click(function(){
			checkLocalIP();
		});
		
		function editLocalIP(){
		...
		function checkLocalIP(){
			...
			if(ip==""){
				$("#localIP").focus();
				messageDialog("请输入本级IP！","alert");
				return;
			}else if(reg==null){
				$("#localIP").focus();
				messageDialog("本级IP不合法！","alert");
				return;
			}else{
				confirmDialog("确定要修改？",function(result){
					if(result){
...
<body>
	<div id="localip_box">
		...
		<input id="edit_localip" type="button" value="修改">
		<hr>
	</div>
	<div id="buttons">
		<label>级联IP设置：</label>
